...
exports.deleteLike = async (req, res) => {
  ...
  try {
    ...
    const checkLike = await connection.dbQuery(
        `SELECT * FROM user_pet_favorite WHERE user_id = '${req.user.id}' AND solid_id = '${req.body.solid_id}'`
    );
    if (checkLike.rows.length === 0) {
      return res.status(400).json({
        status: "fail",
        message: "User has not liked this solid before",
      });
    }

    // If the user has liked the pet before, delete the like
    const solid = await connection.dbQuery(
        `UPDATE pet SET "like" = "like" - 1 WHERE id = '${req.body.solid_id}' RETURNING *`
    );
    const like = solid.rows[0]
